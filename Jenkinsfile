                ...
                withCredentials([
                    ...
                    sh """
                        sshpass -p ${SSH_PASS} ssh -o StrictHostKeyChecking=no ${SSH_USER}@${TARGET_HOST} bash -c "'
                            sudo cp /home/${FTP_USER}/ftp/builds/${IMAGE_TAR} ${DEPLOY_DIR}/
                            sudo mv /home/${FTP_USER}/ftp/builds/${IMAGE_TAR} /home/${FTP_USER}/ftp/builds/smart_vehicle_\\\$(date +%Y-%m-%d_%H-%M-%S).tar
                            sudo cd ${DEPLOY_DIR}
